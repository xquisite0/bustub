...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name != "upper" && func_name != "lower") {
    throw Exception("function name is not 'upper' or 'lower'");
  }
  if (args.size() != 1) {
    throw Exception("too many arguments (expected 1)");
  }
  // throw Exception(fmt::format("func call {} not supported in planner yet", func_name));
  StringExpressionType expr_name;
  if (func_name == "upper") {
    expr_name = StringExpressionType::Upper;
  } else {
    expr_name = StringExpressionType::Lower;
  }

  return std::make_shared<StringExpression>(StringExpression(args[0], expr_name));
}
